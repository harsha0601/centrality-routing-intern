...
## Week 5 — GNN-DQN Evaluation ✅
- Achieved GNN-DQN PDR 81% vs RF baseline 69.8%
...
## Week 8 — Revision: Provenance Audit, Redesign, Final Results ✅
- Discovered and corrected data fabrication in the original ablation (Wilcoxon p=0.0098 was computed on hand-typed values)
- Found and fixed the state bug: original GNN-DQN never received current-node/destination
- Redesigned to per-node Q-values, fixing episode-diversity and destination-blindness bugs
- Original "distance oracle" fix rejected after review — replaced with a symmetric binary destination flag
- Final result: GNN-DQN-v2 = 98.70% ± 1.57 PDR vs DQN-v2 = 57.30% ± 10.32 (p = 0.00195), holding across all 8 pre-registered stress configs
- Full provenance table and committed stats script (`stats_final.py`) trace every number in the paper to a real CSV